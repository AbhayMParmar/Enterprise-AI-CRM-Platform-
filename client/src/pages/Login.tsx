...
type UserRole = 'SalesRep' | 'SalesManager' | 'Admin' | 'SuperAdmin';
...
export const Login = () => {
  ...
  // Strict Validation state tracking
  ...
  // Real Google OAuth using @react-oauth/google useGoogleLogin hook (implicit/access-token flow)
  const googleLoginHook = useGoogleLogin({
    onSuccess: useCallback(async (tokenResponse: Omit<TokenResponse, 'error' | 'error_description' | 'error_uri'>) => {
      setIsGoogleLoading(true);
      setSubmittedMessage(null);
      try {
        // Send Google access_token to our backend which will fetch user profile from Google
    ...
    }, [from, navigate, login, success]),
    onError: useCallback((errorResponse: Pick<TokenResponse, never> | { error: string }) => {
      console.error('Google OAuth error:', errorResponse);
      setSubmittedMessage({ text: 'Google sign-in was cancelled or failed.', type: 'error' });
      setIsGoogleLoading(false);
    }, []),
    scope: 'email profile',
    flow: 'implicit',
  ...
  // Helper flags for simulator mode rendering
  const showDesktopHero = viewport === 'responsive';
  ...
  const getViewportWidthClass = () => {
    switch (viewport) {
      ...
      default:
        return 'w-full max-w-md md:max-w-4xl';
    }
  };

  return (
    <>
      {/* Enterprise Full-Screen Authentication Loading Overlay */}
      <AuthLoadingOverlay
        isVisible={isLoading || isGoogleLoading}
        mode={isGoogleLoading ? 'google' : mode}
      />

      {/* Mobile/Tablet Responsive Design - Hidden on Desktop */}
        ...
        <div className="w-full max-w-2xl mb-3 bg-white/90 backdrop-blur-md p-1.5 rounded-xl border border-slate-200/90 shadow-xs flex items-center justify-between gap-2 overflow-x-auto z-30">
          ...
          <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-lg">
            <button
              ...
              className={`px-2.5 py-1 rounded-md text-[11px] font-semibold flex items-center gap-1 transition-all cursor-pointer ${viewport === 'mobile'
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-md text-[11px] font-semibold flex items-center gap-1 transition-all cursor-pointer ${viewport === 'tablet'
                  ? 'bg-white text-blue-600 shadow-xs'
                  : 'text-slate-500 hover:text-slate-800'
                }`}
            >
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-md text-[11px] font-semibold flex items-center gap-1 transition-all cursor-pointer ${viewport === 'responsive'
                  ? 'bg-white text-blue-600 shadow-xs'
                  : 'text-slate-500 hover:text-slate-800'
                }`}
            >
                    ...
                    <form onSubmit={handleSubmit} className="space-y-2.5" noValidate>
                      ...
                      <div className="flex items-center justify-between pt-0.5">
                        ...
                        <a
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            success('Password recovery placeholder triggered.');
                          }}
        ...
        >
          <Card className="premium-shadow overflow-hidden">
            <CardHeader className="bg-slate-50/50 flex flex-col items-center py-6 text-center border-b border-brand-border">
              ...
              <h1 className="text-xl font-bold text-brand-textPrimary">Sign In</h1>
              <p className="text-xs text-brand-textSecondary mt-1">AI CRM & Sales Management Platform</p>
            </CardHeader>
            ...
            <CardBody className="py-6">
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <div className="flex items-center justify-between text-xs mt-1">
                  <label className="flex items-center gap-1.5 text-brand-textSecondary cursor-pointer">
                    <input type="checkbox" name="rememberMe" className="rounded border-brand-border text-brand-primary focus:ring-brand-primary/20" />
                    <span>Remember me</span>
                  </label>
                  <a href="#forgot" onClick={() => success('Password recovery placeholder triggered.')} className="text-brand-primary hover:underline font-medium">
                    Forgot Password?
                  </a>
                </div>
              ...
              <div className="relative my-6 flex items-center justify-center text-xs text-brand-textSecondary">
                <div className="absolute inset-0 flex items-center">
                  <span className="w-full border-t border-brand-border" />
                </div>
                <span className="relative bg-white px-3 font-semibold uppercase tracking-wider text-[10px]">Or continue with</span>
              </div>
